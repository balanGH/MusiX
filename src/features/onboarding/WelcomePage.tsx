...
export function WelcomePage() {
  ...
  const canPickFolderInput = capabilities().folderInput;
      ...
      <div className="mx-auto flex min-h-full w-full max-w-2xl flex-col justify-center px-6 py-12">
        ...
        <div className="rounded-panel border border-line bg-surface p-5">
          ...
          <div className="mt-4 flex flex-col gap-2 sm:flex-row">
            ...
            {/*
              Only offered where `<input webkitdirectory>` actually works.
              Android's file chooser silently ignores it and returns a normal
              multi-select, so showing a "folder" button there would be a
              button that lies about what it does (spec §41).
            */}
            {!canPickFolder && canPickFolderInput && (
              <Button
                variant="primary"
                size="lg"
                className="flex-1"
            ...
            <Button
              variant={canPickFolder || canPickFolderInput ? 'secondary' : 'primary'}
              size="lg"
              className="flex-1"
              disabled={working}
              loading={working && !canPickFolder && !canPickFolderInput}
              onClick={() => void run(() => addFiles({ folder: false }))}
            >
              <Upload className="h-4 w-4" />
              {canPickFolder || canPickFolderInput ? 'Pick individual files' : 'Select audio files'}
            </Button>
          </div>
